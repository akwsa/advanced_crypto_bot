#!/usr/bin/env python3
# Tujuan: Deteksi support/resistance dari data harga.
# Caller: bot.py, signal pipeline, trading engine.
# Dependensi: pandas/numpy OHLCV.
# Main Functions: class SupportResistanceDetector.
...
class SupportResistanceDetector:
    ...
    def detect_levels(
        ...
        if pair:
            ...
            if manual:
                ...
                manual = self._normalize_levels(manual, current_price)
        ...
        # Keep S/R invariants stable for every detection method:
        # S1 is the nearest support below price, S2 the next lower support;
        # R1 is the nearest resistance above price, R2 the next higher one.
        result = self._normalize_levels(result, current_price)
    ...
    def _normalize_levels(self, result: Dict, current_price: float) -> Dict:
        """Normalize S1/S2/R1/R2 ordering around current price."""
        ...
        support_candidates = [
            _positive_float(normalized.get("support_1")),
            _positive_float(normalized.get("support_2")),
        ]
        resistance_candidates = [
            _positive_float(normalized.get("resistance_1")),
            _positive_float(normalized.get("resistance_2")),
        ]

        supports = [level for level in support_candidates if level is not None]
        resistances = [level for level in resistance_candidates if level is not None]

        if current_price and current_price > 0:
            supports_below = sorted(
                [level for level in supports if level < current_price],
                reverse=True,
            )
            resistances_above = sorted(
                [level for level in resistances if level > current_price],
            )
        else:
            supports_below = sorted(supports, reverse=True)
            resistances_above = sorted(resistances)
        ...
        if current_price and current_price > 0:
            while len(supports_below) < 2:
                fallback = current_price * (0.98 if len(supports_below) == 0 else 0.96)
                supports_below.append(fallback)
            while len(resistances_above) < 2:
                fallback = current_price * (1.02 if len(resistances_above) == 0 else 1.04)
                resistances_above.append(fallback)

        if len(supports_below) >= 1:
            normalized["support_1"] = supports_below[0]
        if len(supports_below) >= 2:
            normalized["support_2"] = supports_below[1]
        if len(resistances_above) >= 1:
            normalized["resistance_1"] = resistances_above[0]
        if len(resistances_above) >= 2:
            normalized["resistance_2"] = resistances_above[1]
