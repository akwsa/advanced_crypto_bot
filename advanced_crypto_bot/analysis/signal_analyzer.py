#!/usr/bin/env python3
# Tujuan: Analisis kualitas signal historis dan metrik win-rate.
# Caller: bot.py command signal quality/report.
# Dependensi: pandas/numpy, signal database data.
# Main Functions: class SignalAnalyzer.
# Side Effects: DB/file read via caller-provided data; no HTTP direct.
"""
...
class SignalAnalyzer:
    ...
    def _get_signals_conn(self):
        ...
        conn = sqlite3.connect(self.signals_db, timeout=30.0)
    ...
    def _get_trades_conn(self):
        ...
        conn = sqlite3.connect(self.trading_db, timeout=30.0)
        conn.row_factory = sqlite3.Row
        conn.execute("PRAGMA journal_mode=WAL")
        conn.execute("PRAGMA busy_timeout=30000")
        return conn
    ...
    def get_signal_quality(self, symbol: str, recommendation: str, days: int = 30) -> Dict:
        ...
        try:
            ...
            try:
                if 'trades_conn' in locals() and trades_conn is not None:
                    trades_conn.close()
            except Exception:
                pass
            try:
                if 'signals_conn' in locals() and signals_conn is not None:
                    signals_conn.close()
            except Exception:
                pass
