...
class SignalDatabase:
    ...
    def _vacuum(self):
        """Run VACUUM on dedicated autocommit connection (outside transaction)."""
        conn = None
        try:
            ...
            conn.execute("VACUUM")
            logger.info("🗜️ Signal DB vacuumed")
            return True
        except Exception as e:
            logger.warning(f"⚠️ Signal DB vacuum skipped: {e}")
            return False
        finally:
            if conn is not None:
                conn.close()
    ...
    def _create_tables(self):
        ...
        with self.get_connection(autocommit=True) as conn:
            ...
            columns = {row["name"] for row in cursor.execute("PRAGMA table_info(signals)").fetchall()}
            if "final_gate_source" not in columns:
                cursor.execute("ALTER TABLE signals ADD COLUMN final_gate_source TEXT")
            if "price_source" not in columns:
                cursor.execute("ALTER TABLE signals ADD COLUMN price_source TEXT")
                ...
                with self.get_connection(autocommit=True) as conn:
                    ...
                    cursor.execute('''
                        INSERT INTO signals (
                            ...
                            analysis, final_gate_source, price_source, signal_time, received_at, received_date
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        signal["symbol"].upper(),
                        ...
                        signal.get("analysis", ""),
                        signal.get("final_gate_source"),
                        signal.get("price_source"),
