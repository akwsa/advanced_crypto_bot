...
class SignalFilterV2:
    ...
    def _default_config(self) -> Dict:
        return {
            ...
            # Filter 5: Confidence Tiers (asimetri Opsi B + Prioritas 1 2026-05-22:
            # BUY-side dilonggarkan supaya selaras dengan signal_rules.py;
            # SELL-side dipertahankan tetap ketat untuk lawan bias bearish ML.)
            "enable_confidence_tiers": True,
            "ml_confidence_min": 0.50,              # Baseline minimum (selaras BUY rules)
            "ml_confidence_buy": 0.50,              # Minimum untuk BUY (selaras signal_rules.BUY_MIN_CONFIDENCE)
            "ml_confidence_strong_buy": 0.64,       # Minimum untuk STRONG_BUY (selaras STRONG_BUY_MIN_CONFIDENCE)
            "ml_confidence_sell": 0.65,             # SELL DIPERTAHANKAN ketat (asimetri Opsi B)
            "ml_confidence_strong_sell": 0.80,      # STRONG_SELL DIPERTAHANKAN ketat (asimetri Opsi B)
            "combined_strength_buy": 0.10,          # Minimum strength untuk BUY (Prioritas 1: 0.30→0.10)
            "combined_strength_strong_buy": 0.35,   # Minimum strength untuk STRONG_BUY (Prioritas 1: 0.60→0.35)
    ...
    def _check_confidence_tiers(self, signal: Dict) -> FilterResult:
        ...
        # --- Cek 4: BUY biasa ---
            ...
            req_ml = self.config["ml_confidence_strong_sell"]
            if ml_confidence < req_ml:
                return FilterResult(
                    filter_name="Confidence Tiers",
                    passed=False,
                    reason=f"STRONG_SELL butuh ML ≥ {req_ml:.0%}, dapat {ml_confidence:.1%}",
                    severity="WARNING",
                    details={"ml_confidence": ml_confidence, "required": req_ml},
                )
        elif recommendation == "SELL":
            req_ml = self.config["ml_confidence_sell"]
            if ml_confidence < req_ml:
                return FilterResult(
                    filter_name="Confidence Tiers",
                    passed=False,
                    reason=f"SELL butuh ML ≥ {req_ml:.0%}, dapat {ml_confidence:.1%}",
                    severity="WARNING",
                    details={"ml_confidence": ml_confidence, "required": req_ml},
                )
