# Tujuan: Client REST private/public Indodax.
# Caller: bot.py, trading engine, scalper, sync scripts.
# Dependensi: requests/httpx-style HTTP, Config API keys, hmac/hashlib.
# Main Functions: class IndodaxAPI.
# Side Effects: HTTP call ke Indodax; no DB direct.
import hashlib
import hmac
import time
import requests
        ...
        try:
            ...
            if ticker:
                ...
                if trade_type.lower() == 'buy':
                    # Unfavourable only when a BUY limit is above market. A maker
                    # BUY below market is intentional and must not be rejected.
                    slippage = max(0.0, (price - market_price) / market_price) if market_price > 0 else 0
                else:  # sell
                    # Unfavourable only when a SELL limit is below market.
                    slippage = max(0.0, (market_price - price) / market_price) if market_price > 0 else 0
