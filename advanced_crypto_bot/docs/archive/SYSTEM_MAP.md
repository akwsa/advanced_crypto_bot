...
├── 📈 quant/             # Quantitative trading modules (NEW)
...
| File | Purpose | Key Classes/Functions |
|------|---------|----------------------|
| `autotrade/trading_engine.py` | Core trading execution, duplicate-position guard, trading-hour gate | `TradingEngine` |
...
| File | Purpose | Key Classes/Functions |
|------|---------|----------------------|
| `quant/__init__.py` | Module exports | All engine classes |
| `quant/mean_reversion.py` | Z-Score mean reversion scoring | `MeanReversionEngine`, `MeanReversionResult` |
| `quant/bayesian_kelly.py` | Adaptive position sizing (Bayesian Kelly) | `BayesianKellyEngine`, `KellyResult` |
| `quant/momentum_factor.py` | Multi-timeframe momentum scoring | `MomentumFactorEngine`, `MomentumResult` |
| `quant/performance_analytics.py` | Sharpe, Sortino, Calmar, drawdown | `PerformanceAnalytics`, `PerformanceMetrics` |
| `quant/dynamic_correlation.py` | Rolling correlation matrix & portfolio heat | `DynamicCorrelationEngine`, `CorrelationCheckResult` |
| `quant/stat_arb.py` | Statistical arbitrage (pair trading) | `StatArbEngine`, `StatArbOpportunity` |

**Dependencies:** `numpy`, `pandas` (sudah ada di project)  
**External Calls:** None (pure computation)

### Integration Points:
- **mean_reversion** → `signals/signal_quality_engine.py` (confluence scoring +0 to +2)
- **mean_reversion** → `signals/signal_pipeline.py` (signal enrichment + confidence boost)
- **bayesian_kelly** → `autotrade/trading_engine.py` (position sizing)
- **momentum_factor** → `core/profit_optimizer.py` (edge score bonus)
- **performance_analytics** → `autotrade/risk_manager.py` (metrics)
- **dynamic_correlation** → `autotrade/risk_manager.py` (portfolio heat)
- **stat_arb** → standalone scanner + `/quant_arb` command
...
| `tests/test_bug_fixes_verification.py` | Regression checks for config, DB, trading engine, signal pipeline fixes |

**Run:** `python -m pytest tests/test_<module>.py` or `python -m unittest tests.test_<module>`
...
3. **Signal Pipeline** → `generate_signal_for_pair()`
   ...
   - **Quant: Z-Score Mean Reversion** via `MeanReversionEngine` (confluence boost)
   - **Quant: Momentum Factor** via `MomentumFactorEngine` (edge scoring)
...
2. **Risk Check** → `RiskManager.validate_trade()`
   - **Quant: Dynamic Correlation** → portfolio heat check
   - **Quant: Performance Analytics** → drawdown-aware gating
3. **Position Sizing** → `TradingEngine.calculate_position_size()`
   - **Quant: Bayesian Kelly** → adaptive per-pair sizing
4. **Order Execution** → `TradingEngine.execute_buy/sell()`
5. **Portfolio Update** → `Portfolio.update()`
6. **Auto-Sell Monitor** → `execute_auto_sell()` (profit target/stop loss)

### Runtime Flow (Statistical Arbitrage)
1. **Price Data Collection** → All watched pairs prices cached
2. **Cointegration Scan** → `StatArbEngine.scan_all_pairs()`
3. **Spread Z-Score** → Detect divergence from mean
4. **Signal Generation** → `LONG_A_SHORT_B` or `SHORT_A_LONG_B`
5. **Exit Signal** → When spread reverts to mean (z < 0.5)
...
- Gunakan `/quant` di Telegram untuk akses quant trading modules
