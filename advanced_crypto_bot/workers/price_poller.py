#!/usr/bin/env python3
# Tujuan: REST price poller berkala untuk update cache harga.
# Caller: bot.py runtime startup.
# Dependensi: requests/Indodax public API, redis_price_cache.
# Main Functions: class PricePoller.
# Side Effects: HTTP calls, Redis/cache writes, background thread.
"""
...
class PricePoller:
    ...
    def __init__(self, bot_instance):
        ...
        self.max_invalid_attempts = 3  # Auto-remove after 3 consecutive failures
    ...
    async def _poll_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # Check pending limit orders every cycle
                try:
                    if hasattr(self.bot, 'check_pending_orders'):
                        await self.bot.check_pending_orders()
                except Exception as e:
                    logger.error(f"❌ Pending order check error: {e}", key="pending_order_err", interval=60)
    ...
    async def _poll_single_pair_async(self, pair):
        ...
        try:
            ...
            if not ticker:
                ...
                await self._track_pair_failure(pair)
                return
            ...
            # Save to database (offloaded to executor to avoid SQLite lock)
            await self._save_price_to_db(pair, {
                'timestamp': datetime.now(),
                'open': last_price,
                'high': last_price,
                'low': last_price,
    ...
    async def _track_pair_failure(self, pair):
        """Track pair failures and REMOVE from watchlist if consistently failing.
        ...
        if self.pair_fail_count[pair] >= self.max_invalid_attempts:
            if pair not in self.invalid_pairs:
                ...
                logger.warning(f"🚫 Pair {pair.upper()} removed from watchlist after {self.pair_fail_count[pair]} failures (invalid/delisted)")
                
                # Remove from bot subscribers (all users)
                removed_users = []
                try:
                    if hasattr(self.bot, 'subscribers') and self.bot.subscribers:
                        for user_id, user_pairs in list(self.bot.subscribers.items()):
                            if pair in user_pairs:
                                self.bot.subscribers[user_id].remove(pair)
                                removed_users.append(user_id)
                                
                    # Remove from database watchlist
                    if hasattr(self.bot, 'db') and self.bot.db:
                        try:
                            await self._remove_watchlist_db(pair)
                        except Exception as db_e:
                            logger.debug(f"DB remove_watchlist_for_pair failed: {db_e}")
                            
                except Exception as e:
                    logger.debug(f"Error removing pair {pair} from watchlist: {e}")
                    
                logger.info(f"💡 Total removed pairs: {len(self.invalid_pairs)} - {', '.join([p.upper() for p in sorted(self.invalid_pairs)])}")

    async def _save_price_to_db(self, pair, ohlcv):
        """Offload DB write to heavy executor to avoid SQLite lock in async loop."""
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                self.bot._heavy_executor,
                lambda: self.bot.db.save_price(pair, ohlcv),
            )
        except Exception as e:
            logger.error(f"❌ DB save_price failed for {pair}: {e}")

    async def _remove_watchlist_db(self, pair):
        """Offload DB write to heavy executor to avoid SQLite lock in async loop."""
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                self.bot._heavy_executor,
                lambda: self.bot.db.remove_watchlist_for_pair(pair),
            )
        except Exception as e:
            logger.debug(f"DB remove_watchlist_for_pair failed: {e}")
